...
[!else]
#ifdef _WIN32_WCE
#error "ATL 不支持 Windows CE 的 HTML 控件。"
#endif
[!endif]
...
protected:
   ...
   {
      return !lstrcmpW(bstr, L"[!output SHORT_NAME] 许可");
   }
   ...
   static BOOL GetLicenseKey(DWORD dwReserved, BSTR* pBstr)
   {
   	  if( pBstr == NULL )
 		return FALSE;
      *pBstr = SysAllocString(L"[!output SHORT_NAME] 许可");
      return TRUE;
   }
...
#ifdef _WIN32_WCE
#error "Windows CE 上不支持公共控件 [!output SUBCLASS_NAME]!"
#endif
[!endif]
...
[!if SUBCLASS_WINDOW && !HTML_CONTROL && !COMPOSITE_CONTROL]
ALT_MSG_MAP(1)
	// 将此替换为超类 [!output SUBCLASS_NAME] 的消息映射项
[!endif]
[!if !SUBCLASS_WINDOW]
[!if !COMPOSITE_CONTROL]
...
[!endif]
	{
		// 不绘制任何内容，因为 WebBrowser 控件将执行绘制任务。
		return S_OK;
	}
